...
import { Todo, User } from '@/zenstack/models';
...
export default function TodoComponent({ value, optimistic }: Props) {
    ...
    const { mutate: updateTodo } = client.todo.useUpdate({ optimisticUpdate: true });
    const { mutate: deleteTodo } = client.todo.useDelete({ optimisticUpdate: true });
