...
import { Space, SpaceUserRole } from '@/zenstack/models';
...
export default function ManageMembers({ space }: Props) {
    ...
    const { mutateAsync: createMember } = client.spaceUser.useCreate();
    const { mutate: deleteMember } = client.spaceUser.useDelete();

    const { data: members } = client.spaceUser.useFindMany({
        where: {
            spaceId: space.id,
        },
        include: {
